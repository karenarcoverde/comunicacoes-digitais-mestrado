...
b_vals    = [1, 2, 4]                  # Bits por símbolo (1→2-PAM, 2→4-PAM, 4→16-PAM)
alpha     = 0.15                       # Roll-off do filtro RRC
sps       = 8                          # Amostras por símbolo
span      = 40                         # Suporte do filtro em símbolos (±20 símbolos)
...
def design_rrc(alpha, span, sps):
    N = span * sps + 1                # Número de taps: suporte total × amostras + 1
    t = np.linspace(-span/2, span/2, N)  # Eixo do tempo de –span/2 a +span/2
    h = np.zeros_like(t)
    for i, ti in enumerate(t):
        if np.isclose(ti, 0):
            # fórmula do RRC no tempo zero
...
# 3) Mapeia bits → símbolos Gray-codificados para M-PAM
...
for EbN0 in EbN0_dB:
    ...
    for ax, b in zip(axes, b_vals):
        ...
        Eb   = np.mean(a**2)/b
        y    = awgn(x, Eb, EbN0)
        y_s  = receive(y, q, sps, delay)[:1000]  # só plotar 1000 pontos
