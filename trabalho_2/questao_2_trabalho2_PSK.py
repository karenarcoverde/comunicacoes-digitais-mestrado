...
for b in b_values:
    ...
    for EbN0_dB in EbN0_dB_range:
        ...
        EbN0 = 10**(EbN0_dB/10)
        # argumento da Q-function aproximada para símbolo
        arg = np.sqrt(2 * np.pi**2 * EbN0 * np.log2(M) / M**2)
        # P_s ≃ 2·Q(arg) = erfc(arg/√2)
    ...
    plt.xlabel('Eb/N0 (dB)')
    plt.ylabel('BER')
    plt.title(f'BER simulada vs teórica para {M}-PSK')
...
for EbN0_dB in EbN0_dB_range:
    ...
    for ax, b in zip(axes, b_values):
        ...
        ax.set_title(f'{M}-PSK — {EbN0_dB} dB')
        ax.set_xlabel('I')
        ax.set_ylabel('Q')
        ax.axis('equal')
        ax.grid(True)
    ...
    fig.savefig(f'constellation_EbN0_{EbN0_dB}dB_PSK.png', dpi=150, bbox_inches='tight')
    plt.close(fig)