...
L = 1                           # número de taps do canal Rayleigh
# --- Canal Rayleigh puro ---
# h[n] = (hR + j hI)/sqrt(2), tamanho L
h = (np.random.randn(L) + 1j*np.random.randn(L)) / np.sqrt(2*L)

rep = 3  # 1 para sem código, 3 para repetição



# 1) Simulação OFDM com QPSK
def simulate_ofdm_qpsk(use_repetition=False):
    ...
    # --- repetição ---
    if use_repetition:
        bits_tx = np.repeat(bits, rep, axis=1)
    else:
        bits_tx = bits
    
    # Passo 3: mapeamento QPSK: s = (1 - 2*d1) + j*(1 - 2*d2)
    sym = (1 - 2*bits_tx[0]) + 1j*(1 - 2*bits_tx[1])
    ...
    sym = sym.reshape((N, S_eff), order='F')  # reorganiza em matriz N×S
    ...
    for snr_db in SNRs_dB:
        ...
        def ofdm_recv(s,type):
            ...
            mat = s.reshape((N+CP, S_eff), order='F')[CP:,:]
            ...
            raw_hat = np.vstack([b1,b2])
            if use_repetition:
                # votação majoritária nos rep símbolos repetidos
                bits_hat = (raw_hat.reshape(rep, -1).sum(axis=0) >= 2).astype(int)
            else:
                # sem código, diretamente o raw_hat
                bits_hat = raw_hat
...
def simulate_ofdm_qam(M, use_repetition=False):
    ...
    sym = modem.modulate(bits_rep).reshape((N, S_eff), order='F')
    ...
    for snr_db in SNRs_dB:
        ...
        def ofdm_recv(s,type):
            ...
            raw_hat = modem.demodulate(y, 'hard') 
            if use_repetition:
                # votação majoritária nos rep símbolos repetidos
                bits_hat = (raw_hat.reshape(rep, -1).sum(axis=0) >= 2).astype(int)
            else:
...
# simula 16-QAM SEM repetição, pega Rayleigh+AWGN (índice 2)
_, _, ber_sem = simulate_ofdm_qam(16, use_repetition=False)
# simula 16-QAM COM repetição, pega Rayleigh+AWGN (índice 2)
_, _, ber_com = simulate_ofdm_qam(16, use_repetition=True)

plt.figure(figsize=(8,5))
plt.semilogy(SNRs_dB, ber_sem, 'bo-',  linewidth=2, label='Sem Correção')
plt.semilogy(SNRs_dB, ber_com, 'rs-',  linewidth=2, label=f'Com Repetição (r={rep})')
plt.grid(which='both', linestyle='--', alpha=0.6)
plt.xlabel('SNR (dB)')
plt.ylabel('BER')
plt.title('BER vs SNR para OFDM com QAM-16 em Canal Rayleigh+AWGN')
plt.legend()   # deixa o Matplotlib escolher o melhor lugar
plt.tight_layout()
plt.savefig('BER_correcao_repeticao.png', dpi=300)
plt.show()
